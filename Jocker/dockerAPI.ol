...
main {
	...
	[ createContainer( request )( response ){
		scope( createContainer )
		{
			...
			if ( is_defined( request.HostConfig.Links ) && #request.HostConfig.Links == 1 ) {
					ln = request.HostConfig.Links;
					undef( request.HostConfig.Links );
					request.HostConfig.Links._ = ln
			};
	...
	[ createNetwork( request )( response ){
		scope( createNetwork )
		{
				...
				if ( is_defined( request.IPAM.Config ) && #request.IPAM.Config == 1 ) {
						ln << request.IPAM.Config;
						undef( request.IPAM.Config );
						request.IPAM.Config._ << ln
				};
